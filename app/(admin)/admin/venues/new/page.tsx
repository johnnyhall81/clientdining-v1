...
export default function NewVenuePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const { error } = await supabase.from('venues').insert({ ...formData, is_active: true })
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
        ...
        <div>
          ...
          <textarea required value={formData.description}
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
            rows={3} placeholder="Brief description of the venue" />
        </div>
        ...
        <div>
          ...
          <input type="text" value={formData.image_hero}
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
            placeholder="https://...supabase.co/storage/v1/object/public/venue-images/venues/hero.jpg" />
        </div>
