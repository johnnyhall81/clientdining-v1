      ...
      {visibleVenues.length > 0 && (
        ...
        >
          {visibleVenues.map(venue => (
            ...
            >
              {/* Top half — hero image */}
              <div style={{ height: '120px', overflow: 'hidden', background: '#F4F2EF' }}>
                {venue.image_hero && (
                  <img src={venue.image_hero} alt={venue.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
              </div>

              {/* Quarter — logo on white */}
              <div style={{ height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 12px', borderBottom: '1px solid #F0EDE9' }}>
                {(venue as any).logo_url ? (
                  <img
                    src={(venue as any).logo_url}
                    alt={venue.name}
                    style={{ maxHeight: '36px', maxWidth: '120px', objectFit: 'contain', filter: 'brightness(0)', opacity: 0.8 }}
                  />
                ) : (
                  <p style={{ fontSize: '12px', color: '#71717a', textAlign: 'center', fontFamily: "Georgia, 'Times New Roman', serif", fontStyle: 'italic', margin: 0, lineHeight: 1.3 }}>
                    {venue.name}
                  </p>
                )}
              </div>
              ...
              {/* Quarter — name + area */}
              <div style={{ height: '60px', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 12px' }}>
