      ...
      <div className="flex flex-col md:flex-row md:items-stretch">
        ...
        <div className="flex-1 px-7 py-7 pr-12 flex flex-col gap-4 min-h-[340px]">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            {venue.address && (
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {venue.phone && (
                  <a href={`tel:${venue.phone}`} className="text-sm font-light text-zinc-500 hover:text-zinc-900 transition-colors">{venue.phone}</a>
                )}
                {venue.phone && venue.booking_email && <span className="text-zinc-300">·</span>}
                {venue.booking_email && (
                  <a href={`mailto:${venue.booking_email}`} className="text-sm font-light text-zinc-500 hover:text-zinc-900 transition-colors">{venue.booking_email}</a>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-sm font-light text-zinc-500">
              {dateStr} · {timeStr} · {partySize} {partySize === 1 ? 'guest' : 'guests'}{hostName ? ` · Hosted by ${hostName}` : ''}
            </span>
            {!isPast && !isCancelled && (
